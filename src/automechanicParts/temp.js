...
[].map(i=>i.properties).flat().map(i=>({name:i.name + ' '+ i.nameId})).map(i=>i.name);
...
[].map(i=>({
  ...
  'Название модификации (UA)': `${i.code} ${i.brandName.replace('FILTERS', "Filters")} Фільтр салону`,
  'Название модификации (RU)': `${i.code} ${i.brandName.replace('FILTERS', "Filters")} Фильтр салона`,
  ...
  Галерея: null,
  'Обзор 360': null,
  Алиас: `${i.code.replace('/', '-')
  .toLowerCase()
  .replace(/[\s\t\n]+/g, '-')}-${i.brandName.toLowerCase().replace(' ', '-')}-filtr-salonu`,
  Ссылка: `https://oluva.shop/${i.code.replace('/', '-')
  .toLowerCase()
  .replace(/[\s\t\n]+/g, '-')}-${i.brandName.toLowerCase().replace(' ', '-')}-filtr-salonu/`,
  'Дата добавления': null,
  'HTML title (UA)': null,
  ...
  'Аксессуары(разделы)': "Інші фільтра/Повітряний фільтр",
	...
	'Вид палива': null,
	Висота: formatMeasurement(i.properties.find(ob=>ob.nameId===103)?.value)? formatMeasurement(i.properties.find(ob=>ob.nameId===103)?.value) : formatMeasurement(i.properties.find(ob=>ob.nameId===172)?.value),
	'Зовнішній діаметр': formatMeasurement(i.properties.find(ob=>ob.nameId===247)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===247)?.value) :formatMeasurement(i.properties.find(ob=>ob.nameId===662)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===662)?.value) : formatMeasurement(i.properties.find(ob=>ob.nameId===663)?.value),
	'Діаметр ущільновальної прокладки': formatMeasurement(i.properties.find(ob=>ob.nameId===3496)?.value),
	'Внутрішня нарізь': formatMeasurement(i.properties.find(ob=>ob.nameId===149)?.value),
	Додатково: i.properties.find(ob=>ob.nameId===318)?.value ? i.properties.find(ob=>ob.nameId===318)?.value : i.properties.find(ob=>ob.nameId===541)?.value,
	Гарантія: null,
	Довжина: formatMeasurement(i.properties.find(ob=>ob.nameId===97)?.value)?formatMeasurement(i.properties.find(ob=>ob.nameId===97)?.value):formatMeasurement(i.properties.find(ob=>ob.nameId===520)?.value),
	Ширина: formatMeasurement(i.properties.find(ob=>ob.nameId===100)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===100)?.value): formatMeasurement(i.properties.find(ob=>ob.nameId===524)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===524)?.value):formatMeasurement(i.properties.find(ob=>ob.nameId===525)?.value),
	'Тип фільтра': null,
	'Внутрішній діаметр': formatMeasurement(i.properties.find(ob=>ob.nameId===246)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===246)?.value) : formatMeasurement(i.properties.find(ob=>ob.nameId===679)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===679)?.value) : formatMeasurement(i.properties.find(ob=>ob.nameId===680)?.value) ,
	'Діаметр трубки виходу пального (випускний)': formatMeasurement(i.properties.find(ob=>ob.nameId===609)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===609)?.value):formatMeasurement(i.properties.find(ob=>ob.nameId===2009)?.value),
	'Діаметр трубки подачі пального (впускний)': formatMeasurement(i.properties.find(ob=>ob.nameId===608)?.value) ? formatMeasurement(i.properties.find(ob=>ob.nameId===608)?.value):formatMeasurement(i.properties.find(ob=>ob.nameId===2006)?.value),
	'Зовнішній діаметр корпуса 2': null,
},
}))
